...
save_filename = 'effnetb3_30_SoftMTL'
...
with open('regulateMatrices_3.json', 'r') as json_file:
    matrices_3 = json.load(json_file)

with open('regulateMatrices_4.json', 'r') as json_file:
    matrices_4 = json.load(json_file)

regulateMat_1 = matrices_1['regulateMat_1']
regulateMat_2 = matrices_2['regulateMat_2']
regulateMat_3 = matrices_3['regulateMat_3']
regulateMat_4 = matrices_4['regulateMat_4']

apply_Matrices = regulateMat_4
...
def balancing_dataset():
    ...
    for domainIdx, domainIter in enumerate(domainList):
        ...
        for categoryIdx, categoryIter in enumerate(categoryList):
            dummyNum = int(apply_Matrices[domainIdx][categoryIdx])
            if dummyNum > 0:
...
def augment_image(image):
    # 이미지를 수평 뒤집기 또는 랜덤 회전 중에서 선택
    if random.random() > 0.5:
        # 50% 확률로 이미지 수평 뒤집기
        return ImageOps.mirror(image)
    else:
        # 50% 확률로 -10도에서 10도 사이로 이미지 회전
        return image.rotate(random.uniform(-10, 10), expand=True)
...
class MultiTaskModel(nn.Module):
    def __init__(self, num_domains=4, num_categories=65):
        super().__init__()
        # 도메인 분류와 카테고리 분류를 위해 두 개의 EfficientNet-B1 모델을 불러오기
        self.base_model_domain = timm.create_model('efficientnet_b3', pretrained=True, num_classes=0, features_only=True)
        self.base_model_category = timm.create_model('efficientnet_b3', pretrained=True, num_classes=0, features_only=True)

        # 각 EfficientNet 모델의 특징 맵 크기 가져오기
        feature_dim_domain = self.base_model_domain.feature_info.channels()[-1]
        feature_dim_category = self.base_model_category.feature_info.channels()[-1]

        # 특징 공유를 위한 결합된 특징 크기 정의
        combined_feature_dim = feature_dim_domain + feature_dim_category

        # 평균 풀링 레이어 정의
        self.pool = nn.AdaptiveAvgPool2d((1, 1))

        # 각 분류 작업에 대한 분류기 레이어 정의
        self.domain_classifier = nn.Linear(combined_feature_dim, num_domains)
        self.category_classifier = nn.Linear(combined_feature_dim, num_categories)

    def forward(self, x):
        # 두 EfficientNet 모델의 마지막 특징 맵 추출
        features_domain = self.base_model_domain(x)[-1]
        features_category = self.base_model_category(x)[-1]

        # 각 특징 맵을 1x1 크기로 축소 (평균 풀링)
        features_domain = self.pool(features_domain)
        features_category = self.pool(features_category)

        # 평탄화하여 벡터로 변환
        features_domain = torch.flatten(features_domain, start_dim=1)
        features_category = torch.flatten(features_category, start_dim=1)

        # 두 모델의 특징을 결합하여 공유된 특징 생성
        shared_features = torch.cat((features_domain, features_category), dim=1)

        # 두 작업에 대해 분류기 레이어에 특징 입력하여 예측 생성
        domain_preds = self.domain_classifier(shared_features)
        category_preds = self.category_classifier(shared_features)
...
class DomainCategoryDataset(Dataset):
    ...
    def _apply_regulation(self, category_images):
        ...
        for (domain_idx, category_idx), images in category_images.items():
            # regulateMat의 값을 반올림하여 조정 수치를 정수로 변환
            adjust_count = round(apply_Matrices[domain_idx][category_idx])
            current_count = len(images)

            if adjust_count > 0 and current_count > 0:
                # 이미지를 추가하기 전에 데이터 증강
                additional_images = [augment_image(Image.open(img)) for img in random.choices(images, k=adjust_count)]
                augmented_images_paths = []
                for index, aug_image in enumerate(additional_images):
                    save_path = f"{images[0][:-4]}_aug_{index}.jpg"  # 새로운 파일 경로 생성
                    aug_image.save(save_path)
                    augmented_images_paths.append(save_path)
                new_samples.extend([(img, domain_idx, category_idx) for img in images + augmented_images_paths])
...
def get_dataset():
    ...
    resize_and_pad = ResizeAndPad(size=(224, 224), fill=0, padding_mode='edge')
    ...
    # Test Dataset Load
    test_dataset = TestCategoryDataset(
        root_dir='C:/Users/USER/PycharmProjects/pythonProject/Office_Home/Splitted_OfficeHomeDataset/test',
        transform=Test_transform)
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False)

    # Train Dataset Load
    dataset = DomainCategoryDataset(
        root_dir='C:/Users/USER/PycharmProjects/pythonProject/Office_Home/Splitted_OfficeHomeDataset/train',
        transform=transform)
...
def train(dataset, train_loader, eval_loader, model, criterion, num_epochs = 30):
    ...
    for epoch in range(num_epochs):
        model.train()
        all_train_domain_labels = []
        all_train_domain_scores = []
        all_train_category_labels = []
        all_train_category_scores = []
        total_loss = 0
        for images, domains, categories in tqdm(train_loader, desc=f'Epoch {epoch+1}/{num_epochs} Training'):
            images, domains, categories = images.to(device), domains.to(device), categories.to(device)
            optimizer.zero_grad()

            train_domain_preds, train_category_preds = model(images)

            loss_domain = criterion(train_domain_preds, domains)
            loss_category = criterion(train_category_preds, categories)
            loss = loss_domain + loss_category
            loss.backward()
            optimizer.step()

            total_loss += loss.item()
            all_train_domain_labels.append(domains.cpu().detach())
            all_train_domain_scores.append(torch.softmax(train_domain_preds, dim=1).cpu().detach())
            all_train_category_labels.append(categories.cpu().detach())
            all_train_category_scores.append(torch.softmax(train_category_preds, dim=1).cpu().detach())

        avg_train_loss = total_loss / len(train_loader)
        train_losses.append(avg_train_loss)

        # 리스트를 텐서로 변환
        all_train_domain_labels = torch.cat(all_train_domain_labels)
        all_train_domain_scores = torch.cat(all_train_domain_scores)
        all_train_category_labels = torch.cat(all_train_category_labels)
        all_train_category_scores = torch.cat(all_train_category_scores)

        # 훈련 mAP 계산
        train_domain_mAP = compute_mAP(all_train_domain_labels, all_train_domain_scores, len(dataset.domain_dict))
        train_category_mAP = compute_mAP(all_train_category_labels, all_train_category_scores,len(dataset.category_dict))

        train_domain_mAPs.append(train_domain_mAP)
        train_category_mAPs.append(train_category_mAP)

        print(f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {total_loss / len(train_loader)}, Train Domain mAP: {100 * train_domain_mAP:.2f}%, Train Category mAP: {100 * train_category_mAP:.2f}%")
    ...
    ##########################################################################################################
        ...
        # 조건이 충족되면 훈련 중단
        if train_domain_mAP > domain_mAP + 0.09 or train_category_mAP > category_mAP + 0.09:
            print(f"Stopping early: Train mAP is more than 9% higher than Eval mAP")
            break
...
def main():
    ...
    model = MultiTaskModel().to(device)
    criterion = nn.CrossEntropyLoss()
    train_losses, eval_losses, domain_mAPs, category_mAPs = train(dataset, train_loader, eval_loader, model, criterion, num_epochs = 10)
