...
f_layer = 5
save_filename = 'effnetb1_30_midsample_Freeze_5layers'
...
class MultiTaskModel(nn.Module):
    def __init__(self, num_domains=4, num_categories=65, freeze_until_layer=f_layer):
        super().__init__()
        self.base_model = timm.create_model('efficientnet_b1', pretrained=True, num_classes=0, features_only=True)

        # EfficientNet B1 모델의 파라미터를 freeze합니다.
        if freeze_until_layer > -1:
            layers = list(self.base_model.children())  # 모델 레이어를 리스트로 가져옵니다.
            # freeze_until_layer 레이어까지의 모든 파라미터를 freeze합니다.
...
def get_dataset():
    ...
    # 데이터셋 레이블 검증
    valid_labels = True
    for _, _, categories in train_loader:
        if not ((categories >= 0) & (categories < 65)).all():
            valid_labels = False
    ...
    if valid_labels:
        ...
        print("Please correct the labels.")

    return train_loader, eval_loader, test_loader, test_dataset, dataset

##########################################################################################################

def train(dataset, train_loader, eval_loader, model, criterion, num_epochs = 10):
...
def main():
    ...
    model = MultiTaskModel().to(device)
    criterion = nn.CrossEntropyLoss()
    train_losses, eval_losses, domain_mAPs, category_mAPs = train(dataset, train_loader, eval_loader, model, criterion, num_epochs = 1)
