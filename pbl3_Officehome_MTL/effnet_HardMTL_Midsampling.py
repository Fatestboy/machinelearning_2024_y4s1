...
with open('regulateMatrices_5.json', 'r') as json_file:
    matrices_5 = json.load(json_file)

regulateMat_4 = matrices_4['regulateMat_4']
regulateMat_5 = matrices_5['regulateMat_5']
...
class MultiTaskModel(nn.Module):
    def __init__(self, num_domains=4, num_categories=65):
        super().__init__()
        self.base_model = timm.create_model('efficientnet_b3', pretrained=True, num_classes=0, features_only=True)
...
def get_dataset():
    ...
    resize_and_pad = ResizeAndPad(size=(300, 300), fill=0, padding_mode='edge')
...
def train(dataset, train_loader, eval_loader, model, criterion, num_epochs = 30):
    ...
    ##########################################################################################################
        ...
        # 조건이 충족되면 훈련 중단
        if train_domain_mAP > domain_mAP + 0.098 or train_category_mAP > category_mAP + 0.09:
